...
- **公共子表达式消除（CSE）** — 消除重复计算的表达式（支持 if-else 分支合并）
- **PassManager 框架** — 优化 pass 独立管理，支持 CFGPass 基类和 CFG 集成
...
# 运行词法分析器单元测试
.\build\Debug\minic_gtests.exe --gtest_filter="LexerUnitTest/*"

# 运行预处理器单元测试
.\build\Debug\minic_gtests.exe --gtest_filter="PreprocessorUnitTest/*"

# 运行语义分析单元测试
.\build\Debug\minic_gtests.exe --gtest_filter="SemanticsUnitTest/*"
```
...
│   ├── Lexer      # 词法分析器
│   ├── Parser     # 语法分析器
│   ├── Semantics  # 语义分析器（ExprVisitor + StmtVisitor）
│   ├── Token      # Token 定义
│   ├── Ast        # AST 节点定义（含 accept 方法）
│   ├── AstAccept  # accept 方法实现
│   ├── ExprVisitor # 表达式 Visitor 接口（19 种节点）
│   ├── StmtVisitor # 语句 Visitor 接口（14 种节点）
│   ├── Preprocessor # 预处理器
│   └── Diagnostics # 诊断引擎
├── backend/       # 代码生成和优化
│   ├── CodeGenerator  # 代码生成器核心（构造、输出工具、全局变量）
│   ├── CodeGenExpr    # 表达式代码生成（Visitor 模式）
│   ├── CodeGenStmt    # 语句代码生成（Visitor 模式）
│   ├── CodeGenCall    # 函数调用代码生成（ABI 处理）
│   ├── CodeGenInit    # 初始化器代码生成（结构体/数组/字符串）
│   ├── CodeGenDwarf   # DWARF 调试信息生成
│   ├── CFG            # 控制流图数据结构
│   ├── CFGBuilder     # 控制流图构建器
│   └── Optimizer      # 优化器
├── linker/        # PE/COFF 链接器
...
tests/
├── minic_regression_tests.cpp   # 端到端回归测试（137 个）
├── lexer_unit_tests.cpp         # 词法分析器单元测试（32 个）
├── preprocessor_unit_tests.cpp  # 预处理器单元测试（23 个）
└── semantics_unit_tests.cpp     # 语义分析单元测试（35 个）

input/             # 测试输入文件
config/            # 配置文件（import_catalog.txt 等）
```
